...
class TestAlbumModel:
    ...
    def test_album_not_equal_to_another_album(self):
        title = "Rust In Peace"
        artist = "Megadeth"
        assert self.album_instance(title, artist) != self.album_instance(
            title, "Metallica"
        )
        assert self.album_instance(title, artist) != self.album_instance(
            "Kill 'Em All", artist
    ...
    @pytest.mark.parametrize("pub_date", [date(1991, 9, 21), present_date(), None])
...
class TestUserModel(AlbumFiltersMixin):
    ...
    def test_add_to_collection_when_album_already_in_collection(
        self, albums_factory, domain_user
    ):
        # Given
    ...
    def test_add_to_collection_when_album_not_in_collection_and_not_on_wishlist(
        self, albums_factory, domain_user
    ):
        # Given
        ...
        actual_albums_in_collection = self.get_albums_in_collection(domain_user)
        assert (
            len(actual_albums_in_collection)
            == len(self.filter_albums_by_ownership(True, albums)) + 1
        )
    ...
    def test_add_to_collection_when_album_on_wishlist(
        self, albums_factory, domain_user
    ):
        # Given
    ...
    def test_add_to_wishlist_when_album_not_on_wishlist(
        self, albums_factory, domain_user
    ):
        # Given
        albums_on_wishlist = albums_factory(owned=False)
    ...
    def test_add_to_wishlist_when_album_already_on_wishlist(
        self, albums_factory, domain_user
    ):
        # Given
        albums_on_wishlist = albums_factory(owned=False)
    ...
    def test_add_to_wishlist_when_album_already_in_collection(
        self, albums_factory, domain_user
    ):
        # Given
        albums = albums_factory(mix=True)
        ...
        with pytest.raises(AlbumAlreadyInCollectionError):
            domain_user.add_to_wishlist(album_from_form)
        actual_albums_on_wishlist = self.get_albums_on_wishlist(domain_user)
        assert len(actual_albums_on_wishlist) == len(
            self.filter_albums_by_ownership(False, albums)
        )
    ...
    def test_edit_album_success(self, albums_factory, domain_user):
        ...
        # Then
        album_from_db = (
            Album.albums.for_user(user=domain_user)
            .filter(title=album_from_form.title, artist=album_from_form.artist)
            .first()
        )
        assert album_from_db.pk == edited_album.pk
        assert album_from_db == album_from_form

    def test_edit_album_duplicate_when_album_from_the_same_set(
        self, albums_factory, domain_user
    ):
        # Given
        albums = albums_factory(mix=True)
        ...
        while other_album == edited_album:
            other_album = choice(albums)
        album_from_form = self.album_instance(
            title=other_album.title, artist=other_album.artist
        )
        # When/Then
        with pytest.raises(
            AlbumAlreadyInCollectionError
            if other_album.owned
    ...
    def test_edit_album_duplicate_when_album_from_different_set(
        self, owned, albums_factory, domain_user
    ):
        # Given
    ...
    def test_move_to_collection_album_from_different_user(
        self, albums_factory, user_factory, domain_user
    ):
        # Given
    ...
    def test_move_to_collection_album_already_in_collection(
        self, albums_factory, domain_user
    ):
        # Given
        albums_in_collection = albums_factory(owned=True)
