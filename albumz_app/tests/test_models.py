...
class TestUserModel(AlbumTestHelpers, TestCase):
    ...
    def test_get_collection_empty_collection(self):
        ...
        # Then
        self.assertEqual(len(albums_in_collection), len(self.get_albums_in_collection()))
        self.assertSetEqual(set(albums_in_collection), set(self.get_albums_in_collection()))
    ...
    def test_add_to_collection_when_album_already_in_collection(self):
        ...
        album_from_form = self.album_from_form(
            album_already_in_collection.title, 
            album_already_in_collection.artist
        )
        # When
        with self.assertRaises(AlbumAlreadyOwnedError):
            self.domain_user.add_to_collection(album_from_form)
        # Then
        self.assertEqual(len(self.get_albums_in_collection()), len(albums_in_collection))
        self.assertSetEqual(
            set(albums_in_collection), 
            set(self.get_albums_in_collection())
        )
    ...
    def test_add_to_collection_when_album_not_in_collection_and_not_on_wishlist(self):
        ...
        album_from_form = self.album_from_form("DefinitelyNotInCollection", "DefinitelyNotInCollectionArtist")
        # When
        self.domain_user.add_to_collection(album_from_form)
        # Then
        self.assertEqual(len(self.get_albums_in_collection()), len(albums_in_collection) + 1)
    ...
    def test_add_to_collection_when_album_on_wishlist(self):
        ...
        album_from_form = self.album_from_form(
            album_on_wishlist.title, 
            album_on_wishlist.artist
        )
        # When
        self.domain_user.add_to_collection(album_from_form)
        # Then
        ...
        self.assertIn(album_from_form, self.get_albums_in_collection())
        self.assertEqual(len(self.get_albums_on_wishlist()), len(albums_on_wishlist) - 1)
    ...
    def test_add_to_wishlist_when_album_already_in_collection(self):
        ...
        album_from_form = self.album_from_form(
            album_in_collection.title, 
            album_in_collection.artist
        )
        # When/Then
        with self.assertRaises(AlbumAlreadyOwnedError):
            self.domain_user.add_to_wishlist(album_from_form)
